...
for i, galaxy in enumerate(['eso443-g024', 'ic1459', 'ic1531']):
# for i in range(3):
	print galaxy
	...
	if overplot:
		for o, color in overplot.iteritems():
			add_(o, color, axs[2*i,1], galaxy)


	axs[2*i+1,1] = plot_velfield_nointerp(D.x, D.y, D.bin_num, D.xBar, D.yBar, 
		unc_met, header, vmin=0, vmax=0.67+2.25, cmap='gnuplot2', 
		flux_unbinned=D.unbinned_flux, signal_noise=D.SNRatio, 
		signal_noise_target=SN_target, ax=axs[2*i+1,1])

	# Alpha
	axs[2*i,2] = plot_velfield_nointerp(D.x, D.y, D.bin_num, D.xBar, D.yBar, 
		alp, header, vmin=-0.3, vmax=0.5, cmap='gnuplot2', 
		flux_unbinned=D.unbinned_flux, signal_noise=D.SNRatio, 
		signal_noise_target=SN_target, ax=axs[2*i,2])
	if overplot:
		for o, color in overplot.iteritems():
			add_(o, color, axs[2*i,2], galaxy)


	axs[2*i+1,2] = plot_velfield_nointerp(D.x, D.y, D.bin_num, D.xBar, D.yBar, 
...
fig.savefig('%s/pop1.png' % (out_dir), bbox_inches='tight')
