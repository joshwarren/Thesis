...
% Finish this off WITHOUT looking at Becky's thesis			
			Markov chain Monte--Carlo (MCMC) fitting is useful as it allows for the efficent exploration of parameter space by avoiding regions with low probablities (high $\chi^2_\text{red}$). A large number of `walkers' are




			We use \textsc{emcee}, a \textsc{python} package for Markov chain Monte--Carlo (MCMC) fitting \citep{Foreman-Mackey2013}, to find the most-likely SSP model, by comparing the absorption line strength indices measured (with the method described above in Section \ref{subsubsec:Absorption}) to those of the SSP models. 



			\textsc{emcee} is an affine-invarinet ensemble MCMC sampler meaning that we can take the standard deviation of the positions of the walkers in the MCMC code as the uncertainty on a given index.
